...
public interface SaleRepository extends JpaRepository<Sale, Long> {
  Page<Sale> findAllBySaleTimeBetweenAndAmountGreaterThanEqual(LocalDateTime saleTimeAfter, LocalDateTime saleTimeBefore,
                                                               BigDecimal amount, Pageable pageable);
  ...
  @Query("select paymentType as paymentType, count(1) as totalCount, sum(amount) as totalAmount, " +
      ...
      "  from Sale where saleTime between :saleTimeAfter and  :saleTimeBefore and deleted<>true " +
      "  and amount >= :amount group by paymentType")
