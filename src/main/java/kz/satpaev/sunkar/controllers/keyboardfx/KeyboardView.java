package kz.satpaev.sunkar.controllers.keyboardfx;
...
import kz.satpaev.sunkar.controllers.keyboardfx.Keyboard.SpecialKey;
import kz.satpaev.sunkar.controllers.keyboardfx.Keyboard.SpecialKey.Type;
import kz.satpaev.sunkar.controllers.keyboardfx.skins.KeyboardViewSkin;
...
public class KeyboardView extends Control {
     ...
     */
    public KeyboardView(String... name) {
        ...
        getStylesheets().add(Objects.requireNonNull(KeyboardView.class.getClassLoader().getResource("css/keyboard.css")).toExternalForm());

        darkModeProperty().addListener(it -> {
            if (isDarkMode()) {
                getStylesheets().add(Objects.requireNonNull(KeyboardView.class.getClassLoader().getResource("css/keyboard-dark.css")).toExternalForm());
            } else {
                getStylesheets().remove(Objects.requireNonNull(KeyboardView.class.getClassLoader().getResource("css/keyboard-dark.css")).toExternalForm());
            }
        });
        ...
        for (int i = 0; i < name.length; i++) {
            getKeyboards().add(loadKeyboard(name[i]));
        }
    }
    ...
    public Keyboard loadKeyboard(String fileName) {
        try (InputStream in = KeyboardView.class.getClassLoader().getResourceAsStream(fileName)) {
            if (in != null) {
